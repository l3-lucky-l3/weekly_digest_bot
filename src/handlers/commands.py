...
📅 Расписание:
• Пн 10:00 - цели/блокеры недели (в топик announce)
• Пт 19:00 - Weekly Digest (в топик Анонсы)

📋 Управление топиками:
/addtopic <название> - добавить текущий топик для мониторинга
/deletetopic - удалить текущий топик из мониторинга
/select_announce_topic <название> - установить текущий топик для постов в понедельник
/select_digest_topic <название> - установить текущий топик для дайджестов в пятницу
...
/show_prompts - показать текущие пром_пты_
...
async def cmd_setprompt(message: Message, state: FSMContext, db):
    ...
    try:
        ...
        await message.answer(
            f"✏️ Настройка промпта для {prompt_type_names[prompt_type]}\n\n"
            f"📝 Текущий промпт:\n{current_prompt or 'Не установлен'}\n\n\n"
            f"📨 Отправьте новый промпт в следующем сообщении:\n"
